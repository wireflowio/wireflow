...
func SetRoute() RouterPrintf {
	return func(action, address, name string) {
		...
		case "add":
			...
			klog.Infof("add route %s -net %v dev %s", action, internal.GetCidrFromIP(address), name)
		case "delete":
			internal.ExecCommand("/bin/sh", "-c", fmt.Sprintf("route %s -net %v dev %s", action, internal.GetCidrFromIP(address), name))
			klog.Infof("delete route %s -net %v dev %s", action, internal.GetCidrFromIP(address), name)
		}
