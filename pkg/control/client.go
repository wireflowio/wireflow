...
var (
	_ internal.ControlClient = (*Client)(nil)
)
...
type Client struct {
	...
	ch              chan *linkconn.DirectChecker
	...
	proberManager   *linkconn.ProberManager
	proberMux       sync.Mutex
	turnClient      *linkconn.Client
}

type ClientConfig struct {
	Pm              *config.PeersManager
	Conf            *config.LocalConfig
	PeerCh          chan *linkconn.DirectChecker
	Agent           *ice.Agent
	UdpMux          *ice.UDPMuxDefault
	UniversalUdpMux *ice.UniversalUDPMuxDefault
	Km              *internal.KeyManager
...
func (c *Client) probeConn(agent *ice.Agent, peer *config.Peer) error {
	...
	directContact := func(agent *ice.Agent, peer *config.Peer) error {
		...
		if prober == nil {
			...
			prober = linkconn.NewProber(&linkconn.ProberConfig{
	...
	relayConact := func(peer *config.Peer) error {
		...
		relayAddr, err := linkconn.AddrToUdpAddr(relayInfo.RelayConn.LocalAddr())

		relayOffer := linkconn.NewOffer(relayInfo.MappedAddr, *relayAddr, c.agentManager.GetLocalKey(), linkconn.OfferTypeRelayOffer)
