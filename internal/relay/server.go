...
func (w *WRRPManager) Register(streamId uint64, stream Stream) {
	w.mu.Lock()
	defer w.mu.Unlock()
	w.streams[streamId] = &Session{
		ID:     streamId,
		Stream: stream,
		Type:   "WRRP",
	}
...
func (w *WRRPManager) RegisterQUIC(id uint64, ctrl Stream, conn *quic.Conn) {
	w.mu.Lock()
	defer w.mu.Unlock()
	w.streams[id] = &Session{
		ID:     id,
		Stream: ctrl,
		Type:   "QUIC",
	}
...
func (w *WRRPManager) Get(id uint64) *Session {
	w.mu.Lock()
	defer w.mu.Unlock()
	return w.streams[id]
}
