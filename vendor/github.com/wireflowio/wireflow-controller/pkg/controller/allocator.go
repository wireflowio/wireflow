...
func (a *IPAllocator) AllocateIP(network *wireflowv1alpha1.Network, nodeName string) (string, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
...
// ReleaseIP 释放 IP 地址
func (a *IPAllocator) ReleaseIP(network *wireflowv1alpha1.Network, ip string) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *IPAllocator) CountAvailableIPs(network *wireflowv1alpha1.Network) (int, error) {
	_, ipNet, err := net.ParseCIDR(network.Spec.CIDR)
	if err != nil {
		return 0, err
	}
