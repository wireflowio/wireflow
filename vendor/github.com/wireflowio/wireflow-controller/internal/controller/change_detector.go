...
import (
	...
	"github.com/wireflowio/wireflow-controller/internal/model"
...
func (d *ChangeDetector) findChangedNodes(ctx context.Context, oldNodeCtx *NodeContext, added, removed []string, req ctrl.Request) ([]*model.Peer, []*model.Peer) {
	logger := logf.FromContext(ctx)
	addedPeers := make([]*model.Peer, 0)
	removedPeers := make([]*model.Peer, 0)

	//1、删除的节点
	for _, remove := range removed {
		for _, node := range oldNodeCtx.Nodes {
			if remove == node.Name {
				removedPeers = append(removedPeers, &model.Peer{
					Name:       node.Name,
	...
	for _, name := range added {
		...
		addedPeers = append(addedPeers, &model.Peer{
			Name:       node.Name,
			AppID:      node.Spec.AppId,
			Address:    node.Status.AllocatedAddress,
			PublicKey:  node.Spec.PublicKey,
...
func (d *ChangeDetector) buildFullConfig(node *wireflowv1alpha1.Node, context *NodeContext, changes *model.ChangeDetails, version string) (*model.Message, error) {
	// 生成配置版本号
	msg := &model.Message{
		EventType:     model.EventTypeNodeUpdate, // 统一使用 ConfigUpdate
	...
	if context.Network != nil {
		...
		for _, peer := range context.Nodes {
			...
			msg.Network.Peers = append(msg.Network.Peers, &model.Peer{
				Name:       peer.Name,
		...
		for _, policy := range context.Policies {
			msg.Network.Policies = append(msg.Network.Policies, &model.Policy{
